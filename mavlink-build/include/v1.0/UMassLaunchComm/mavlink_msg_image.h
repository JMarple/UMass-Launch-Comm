...
#define MAVLINK_MSG_ID_IMAGE_LEN 210
#define MAVLINK_MSG_ID_101_LEN 210

#define MAVLINK_MSG_ID_IMAGE_CRC 57
#define MAVLINK_MSG_ID_101_CRC 57
...
#define MAVLINK_MESSAGE_INFO_IMAGE { \
	"IMAGE", \
	5, \
	{  { "fileSize", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_image_t, fileSize) }, \
         { "segment", NULL, MAVLINK_TYPE_UINT16_T, 0, 4, offsetof(mavlink_image_t, segment) }, \
         { "image", NULL, MAVLINK_TYPE_UINT16_T, 0, 6, offsetof(mavlink_image_t, image) }, \
         { "bytes", NULL, MAVLINK_TYPE_UINT16_T, 0, 8, offsetof(mavlink_image_t, bytes) }, \
         { "data", NULL, MAVLINK_TYPE_CHAR, 200, 10, offsetof(mavlink_image_t, data) }, \
         } \
}
...
static inline uint16_t mavlink_msg_image_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint16_t segment, uint16_t image, uint32_t fileSize, uint16_t bytes, const char *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_IMAGE_LEN];
	_mav_put_uint32_t(buf, 0, fileSize);
	_mav_put_uint16_t(buf, 4, segment);
	_mav_put_uint16_t(buf, 6, image);
	_mav_put_uint16_t(buf, 8, bytes);
	_mav_put_char_array(buf, 10, data, 200);
...
{
	return mavlink_msg_image_pack(system_id, component_id, msg, image->segment, image->image, image->fileSize, image->bytes, image->data);
}
...
{
	return mavlink_msg_image_pack_chan(system_id, component_id, chan, msg, image->segment, image->image, image->fileSize, image->bytes, image->data);
}
...
static inline void mavlink_msg_image_send(mavlink_channel_t chan, uint16_t segment, uint16_t image, uint32_t fileSize, uint16_t bytes, const char *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_IMAGE_LEN];
	_mav_put_uint32_t(buf, 0, fileSize);
 ...
 */
static inline void mavlink_msg_image_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint16_t segment, uint16_t image, uint32_t fileSize, uint16_t bytes, const char *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field fileSize from image message
 *
 * @return 
 */
static inline uint32_t mavlink_msg_image_get_fileSize(const mavlink_message_t* msg)
{
...
{
	return _MAV_RETURN_char_array(msg, data, 200,  10);
}
...
static inline void mavlink_msg_image_decode(const mavlink_message_t* msg, mavlink_image_t* image)
{
	...
	image->fileSize = mavlink_msg_image_get_fileSize(msg);
